...
import { FaArrowLeft, FaClock, FaMapMarkerAlt, FaPlus, FaBook } from "react-icons/fa";
...
export default function MataKuliahTab({ setCurrentTab }: MataKuliahTabProps) {
    ...
    const fetchJadwal = async () => {
        ...
        const { data, error } = await supabase
            .from('jadwal')
            .select(`
                id, hari, jam_mulai, jam_selesai, ruangan,
    ...
    const handleAddJadwal = async (e: React.FormEvent) => {
        ...
        setIsSubmitting(true);
        const { error } = await supabase.from('jadwal').insert([newJadwal]);
        setIsSubmitting(false);
                        ...
                        {mkList.map((m) => (
                            <tr key={m.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition">
                                ...
                                <td className="p-6 text-center">
                                    <button className="bg-red-50 text-red-500 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-red-100">Hapus</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {isAddJadwalModalOpen && (
                <div className="fixed inset-0 bg-slate-900/40 z-[300] flex items-center justify-center p-4 backdrop-blur-md">
                    <form onSubmit={handleAddJadwal} className="bg-white/90 backdrop-blur-xl w-full max-w-lg rounded-[3rem] p-10 shadow-2xl border border-white max-h-[90vh] overflow-y-auto custom-scroll">
                        <div className="text-center mb-8">
                            <div className="w-16 h-16 bg-indigo-100 text-indigo-600 rounded-[1.5rem] flex items-center justify-center mx-auto mb-4">
                                <FaClock className="text-2xl" />
                            </div>
                            <h3 className="text-2xl font-extrabold text-slate-800 uppercase tracking-tighter">Jadwal Baru</h3>
                        </div>
                        <div className="space-y-4 mb-8">
                            <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button type="button" onClick={() => setIsAddJadwalModalOpen(false)} disabled={isSubmitting} className="py-5 bg-slate-200 text-slate-600 font-extrabold rounded-2xl uppercase text-[10px] tracking-widest hover:bg-slate-300">Batal</button>
